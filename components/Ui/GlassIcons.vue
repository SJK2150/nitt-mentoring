<template>
  <div :class="['glass-icon-btns', className]">
    <button 
      v-for="(item, index) in items" 
      :key="index"
      :class="['glass-icon-btn', item.customClass]"
      :aria-label="item.label"
      type="button"
      @click="item.onClick">
      <span class="glass-icon-btn__back" :style="getBackgroundStyle(item.color)"></span>
...
const gradientMapping: Record<string, string> = {
  ...
  maroon: 'linear-gradient(hsl(350, 70%, 40%), hsl(340, 70%, 35%))'
};
